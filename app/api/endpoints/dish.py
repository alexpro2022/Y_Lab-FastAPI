...
async def get_all_(submenu_id: str, submenu_service: submenu_service):
    submenu = await submenu_service.get(id=submenu_id)
...
async def create_(submenu_id: str,
    ...
    submenu = await submenu_service.get(id=submenu_id, exception=True)
    return await dish_service.create(**payload.model_dump(), submenu_id=submenu.id)
...
async def update_(item_id: str,
                  ...
                  dish_service: dish_service):
    return await dish_service.update(id=item_id, **payload.model_dump())
